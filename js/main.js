$(function(){
  ...
  // 1. Логика для перехода между вкладками слайдеру /begine-code
  //  Ссылки изменяют свои свойства вместо перехода на другую страницу, ссылки выполняют переход статически
   $('.info-tabs__item').on('click', function(e) {
     e.preventDefault(); 

    // Удаляю линию подчеркивания с первой вкладки класса "info-tabs__item" и определяю ее как активную для той вкладки на которою кликаю 
   ...
   // Логика для перехода между вкладками слайдеру /end-code

   // 2. Логика для слайдера /begine-code
   $('.info-slider, .clients-slider').slick({
     fade: true, // Плавность смены слайдов
    //  Стрелка (svg) влево (предыдущий слайд)
    ...
    //  Стрелка (svg) вправо (следующий слайд)
